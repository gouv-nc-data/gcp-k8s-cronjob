...
| [google_storage_bucket_iam_member.staging_bucket_get](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/storage_bucket_iam_member) | resource |
...
| <a name="input_pod_failure_policy_rules"></a> [pod\_failure\_policy\_rules](#input\_pod\_failure\_policy\_rules) | Règles `podFailurePolicy` du Job : décident si l'échec d'un pod compte<br/>dans `backoff_limit`. Cas d'usage principal : ignorer les préemptions et<br/>évictions GKE (condition `DisruptionTarget`) pour qu'un pod tué avant<br/>d'avoir tourné soit relancé, sans pour autant retenter les vraies erreurs<br/>applicatives. Nécessite `restart_policy = "Never"` (défaut) et Kubernetes >= 1.31. | <pre>list(object({<br/>    action = string<br/>    on_pod_condition = optional(list(object({<br/>      type   = string<br/>      status = optional(string, "True")<br/>    })), [])<br/>    on_exit_codes = optional(object({<br/>      container_name = optional(string)<br/>      operator       = string<br/>      values         = list(number)<br/>    }))<br/>  }))</pre> | `[]` | no |
...
| <a name="input_resources_limits"></a> [resources\_limits](#input\_resources\_limits) | Limites de ressources | <pre>object({<br/>    memory            = string<br/>    cpu               = string<br/>    ephemeral-storage = optional(string, "2Gi")<br/>  })</pre> | <pre>{<br/>  "cpu": "1000m",<br/>  "ephemeral-storage": "2Gi",<br/>  "memory": "2Gi"<br/>}</pre> | no |
| <a name="input_resources_requests"></a> [resources\_requests](#input\_resources\_requests) | Ressources demandées | <pre>object({<br/>    memory            = string<br/>    cpu               = string<br/>    ephemeral-storage = optional(string, "1Gi")<br/>  })</pre> | <pre>{<br/>  "cpu": "500m",<br/>  "ephemeral-storage": "1Gi",<br/>  "memory": "512Mi"<br/>}</pre> | no |
| <a name="input_restart_policy"></a> [restart\_policy](#input\_restart\_policy) | Politique de redémarrage du conteneur (Never = pas de restart in-Pod, OnFailure = restart dans le même Pod) | `string` | `"Never"` | no |
